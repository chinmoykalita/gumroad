  ...
  context "with subscription products" do
    ...
    context "with no churn data" do
      before do
        ...
        purchase.update_columns(purchase_state: "successful", succeeded_at: Time.utc(2025, 5, 1, 12, 0, 0), created_at: Time.utc(2025, 5, 1))
  ...
  private
    def create_churned_subscription(product, price_cents, churn_date:)
      travel_to churn_date - 3.months do
        ...
        purchase.skip_preparing_for_charge = true if purchase.respond_to?(:skip_preparing_for_charge=)
        purchase.save!(validate: false)
        purchase.update_columns(purchase_state: "successful", succeeded_at: Time.current + 1.hour, created_at: Time.current)
        ...
        2.times do |i|
          ...
          purchase.skip_preparing_for_charge = true if purchase.respond_to?(:skip_preparing_for_charge=)
          purchase.save!(validate: false)
          purchase.update_columns(purchase_state: "successful", succeeded_at: Time.current + (i + 1).month + 1.hour, created_at: Time.current + (i + 1).month)
        end
    ...
    def create_active_subscriptions(product, count:, price_cents:)
      travel_to Time.utc(2025, 1, 1) do
        count.times do |i|
          ...
          purchase.skip_preparing_for_charge = true if purchase.respond_to?(:skip_preparing_for_charge=)
          purchase.save!(validate: false)
          purchase.update_columns(purchase_state: "successful", succeeded_at: Time.current + i.days + 1.hour, created_at: Time.current + i.days)
          ...
          3.times do |payment_idx|
            ...
            purchase.skip_preparing_for_charge = true if purchase.respond_to?(:skip_preparing_for_charge=)
            purchase.save!(validate: false)
            purchase.update_columns(purchase_state: "successful", succeeded_at: Time.current + (payment_idx + 1).month + i.days + 1.hour, created_at: Time.current + (payment_idx + 1).month + i.days)
          end
        end
      end
    end
